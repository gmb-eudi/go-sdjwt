...
// and claim-name identifiers — NOT algorithm identifiers (algorithm literals
...
// dereferences x5u/jku or the system pool.
...
// VerifiedCredential field, so filtering Claims would lose them (see the
// README Decisions section). Registered members that DO have a dedicated field here
// (cnf, status) are ALSO surfaced via that field. DecoyDigests counts decoy
// digests seen (SD-JWT privacy feature; the README Decisions require surfacing
...
// StatusRef is the status_list reference of a credential
// ([Token Status List §5]). Fetch/verify/lookup live in go-statuslist; this is extraction
// only.
